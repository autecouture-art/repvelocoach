...
export function calculateLVP(
  ...
  // v1rm: 理論上の1RM時の速度
  // 以前のコードでは intercept（V0 / 負荷0の速度）を代入していましたが、これは誤りです。
  // 1RM時の速度は、定義上、渡された MVT (Minimum Velocity Threshold) と一致すべきです。
  const v1rm = mvt;

  return {
    lift: '', // To be filled by caller
    vmax,
    v1rm,     // 理論上の1RM時の速度（回帰式の外挿値）
    mvt,      // 実際に測定された限界速度（引数で渡された値）
    slope,
    intercept,
...
export function getExpectedROM(category: string): number {
  switch (category) {
    ...
    case 'bench': return 35;
    case 'deadlift': return 45;
    case 'press': return 40;
    case 'pull': return 45;
    default: return 30;
  }
}
